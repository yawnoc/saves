...
| Android Studio | Emulator terminated with exit code 139 | Edit `~/.android/avd/name.avd` to have `hw.gpu.mode=host` (i.e. Graphics acceleration = Hardware) | |
...
| Linux | LUKS storage | `fdisk`, `cryptsetup`, and `mkfs.ext4` | <https://gist.github.com/JChristensen/02f97ee2acfb22fa48678853a424c890> |
...
| macOS | Bash not up to date | `brew install bash` then append `/opt/homebrew/bin/bash` to `/etc/shells` then `chsh -s /opt/homebrew/bin/bash` |
...
| WSL | Windows broken with [WARN: COPY MODE] | `wsl --shutdown` then restart | <https://github.com/microsoft/wslg/discussions/312#discussioncomment-7607596> |
